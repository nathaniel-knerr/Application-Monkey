...
def main(job_description):
    ...
    skill_names = db.getColumn("skill_name", "Skills")
    skill_names_str = ", ".join(skill_names)

    category_names = db.getColumn("category_name", "Categories")
    category_names_str = ", ".join(category_names)

    value_names = db.getColumn("value_name", "Values")
    ...
    prompt = [
        {
            ...
            Here are the skills: {skill_names_str}.
            Here are the categories: {category_names_str}.
            Here are the values: {value_names_str}.
            """
        },
        {
            "role": "user",
    ...
    most_relevant_experiences = db.getMostRelevantExperiences(job_skill_names, job_category_names)
    ...
    Start year: {db.getExperienceData(most_relevant_experience_1, "start_year")};
    Start month: {db.getExperienceData(most_relevant_experience_1, "start_month")};
    End year: {db.getExperienceData(most_relevant_experience_1, "end_year")};
    End month: {db.getExperienceData(most_relevant_experience_1, "end_month")};
    Experience URL: {db.getExperienceData(most_relevant_experience_1, "experience_url")};
    2nd most relevant: Experience: {most_relevant_experience_2};
    Start year: {db.getExperienceData(most_relevant_experience_2, "start_year")};
    Start month: {db.getExperienceData(most_relevant_experience_2, "start_month")};
    End year: {db.getExperienceData(most_relevant_experience_2, "end_year")};
    End month: {db.getExperienceData(most_relevant_experience_2, "end_month")};
    Experience URL: {db.getExperienceData(most_relevant_experience_2, "experience_url")}
    """

    personal_info = db.getPersonalInformation()
    personal_info_str = f"""
    ...
    pdf.output("public/cover-letters/" + output_file_name)
    """
